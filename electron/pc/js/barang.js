...
const API_URL = `http://${SERVER_IP}:1987/maju/api/barang.php`;
const API_URL_KATEGORI = `http://${SERVER_IP}:1987/maju/api/kategori.php`;
...
async function fetchAndDisplayBarang(keyword = "", page = 1) {
    tableBody.innerHTML = `<tr><td colspan="7" style="text-align: center; padding: 20px;">Memuat data...</td></tr>`;
    ...
    try {
        ...
        tableBody.innerHTML = `<tr><td colspan="7" style="text-align: center; padding: 20px; color: red;">Gagal mengambil data dari server.</td></tr>`;
    }
}
...
function renderTable(data) {
    if (!data.length) {
        tableBody.innerHTML = `<tr><td colspan="7" style="text-align: center; padding: 20px;">Tidak ada data.</td></tr>`;
        return;
    }
    const startIndex = (currentPage - 1) * itemsPerPage;
    tableBody.innerHTML = data.map((b, i) => `
        <tr onclick="warnabaris(this)">
          <td>${startIndex + i + 1}</td>
          <td>${b.barcode ? b.barcode : '-'}</td>
          <td>${b.nama_barang}</td>
          <td style="text-align: right;">${formatRupiah(b.harga_hna)}</td>
          <td style="text-align: right;">${b.stok}</td>
          <td>${b.satuan}</td>
          <td style="text-align: center;">
            <button onclick="editBarang(${b.barang_id})">Edit</button>
            <button onclick="hapusBarang(${b.barang_id})">Hapus</button>
          </td>
        </tr>
      `).join("");
}
...
btnTambah.onclick = () => {
    ...
    stok.parentElement.style.display = '';
    stok.required = true;
    stok.value = 0;
...
window.editBarang = async (id) => {
    try {
        ...
        if (result.status === "success" && result.data) {
            ...
            // stok.parentElement.style.display = 'none';
            // stok.required = false;
            modal.style.display = "flex";
        }
    } catch {
...
document.addEventListener("click", (event) => {
    if (!event.target.closest("#barangTable")) {
        clearSelectedRows();
    }
});

...
class MYPDF extends TCPDF
{
    ...
    public function Header()
    {
        ...
        $dueDate = (clone $invoiceDate)->modify('+21 days');

        // Baris 1
        $this->SetXY($rightX, $rightY);
        $this->Cell($rightW, 4, "Tanggal Invoice", 0, 0, 'L');
        $this->Cell(5, 4, ":", 0, 0, 'C');
        $this->Cell(30, 4, $invoiceDate->format('d M Y'), 0, 1);

        // Baris 2
        $this->SetX($rightX);
        $this->Cell($rightW, 4, "Jatuh Tempo", 0, 0, 'L');
        $this->Cell(5, 4, ":", 0, 0, 'C');
        $this->Cell(30, 4, $dueDate->format('d M Y'), 0, 1);
        ...
        $cols = [10, 96, 10, 22, 22];
        $headers = ['No', 'Nama Barang', 'Qty', 'Harga', 'Total'];
...
foreach ($penjualan_details as $i => $item) {
    ...
    $pdf->MultiCell($cols[2], $row_height, $qty, 1, 'C', false, 0);
    $pdf->MultiCell($cols[3], $row_height, number_format($price, 0), 1, 'R', false, 0);
